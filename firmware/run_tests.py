...
def independent_verifier_vectors() -> bool:
    """The committed JSON blob must be what attest.py actually packs.

    Coordinators (HEIR, a multisig ceremony) verify this file. If it is
    regenerated by a third-party implementation, it has proven nothing.
    """
    import json
    import attest

    path = HERE / "vectors" / "attest-v1.json"
    if not path.exists():
        print("    firmware/vectors/attest-v1.json is missing")
        print("    run: python firmware/attest.py --export-vectors "
              "firmware/vectors/attest-v1.json")
        return False
    stored = json.loads(path.read_text())
    live = attest.export_vectors()
    if stored["records"][0]["blob"] != live["records"][0]["blob"]:
        print("    firmware/vectors/attest-v1.json is stale")
        print("    run: python firmware/attest.py --export-vectors "
              "firmware/vectors/attest-v1.json")
        return False
    rec = stored["records"][0]
    a = attest.Attestation.unpack(bytes.fromhex(rec["blob"]))
    v = attest.verify(
        a,
        bytes.fromhex(rec["attestPub"]),
        bytes.fromhex(rec["sighash"]),
        min_counter=41,
        allowed_fw=[bytes.fromhex(rec["fwHash"])],
        allowed_cal=[bytes.fromhex(rec["calHash"])],
    )
    if not v.ok:
        print(f"    stored blob does not verify: {v}")
        return False
    return True
...
def main() -> int:
    ...
    IN_PROCESS = [
        ...
        ("independent-verifier vectors match attest.py", independent_verifier_vectors),
