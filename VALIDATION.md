...
`python firmware/run_tests.py` — 41 suites, no hardware required.
...
| A fresh clone | `git clone`, install, `run_tests.py` | 41 suites pass; every path and command the docs name resolves; the LFS mesh arrives as a mesh; all four generators reproduce `models/`, `diagrams/` and `viewer/` byte for byte |
