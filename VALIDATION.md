...
| Quorum on the confirmation screen | "MULTISIG 2 of 3" and "signature 1 of 2", rebuilt from the registered co-signers, never read off the host's script |
...
| v2 signed | Same signature as v0, returned as v2 and never silently downgraded |
...
| Nothing to sign | A PSBT holding none of our keys is refused instead of silently returned unchanged |
...
| **Every parser, against hostile bytes** | `test_fuzz.py`, nine entry points against mutated noise, and 6,000 corrupted-but-valid PSBTs driven all the way to a drawn screen. Each is held to the exceptions it declares; `classify` and `verify_blob` may not raise at all | Deterministic, one fixed seed. **Found one escape**: `parse_eth_request` did not check that the JSON document was an object, so `4` raised `TypeError`, outside what `app.sign_eth` catches, turning a refusal into the "internal error" screen this design says a refusal must never be. `ops.parse` had always made that check |
| **Encoders, over random valid inputs** | Round trips at volume, past the handful a vector file holds: every script type on every network, 1,164 single-character address mutations, 1,500 nested RLP structures, BIP-39 at every length, QR frames shuffled and duplicated | All clean. Also measures something worth knowing rather than asserting something BIP-39 never promised: a single wrong word is undetectable about once in 2^k, which is **6% at twelve words and 0.4% at twenty-four**. The arithmetic behind `provision.py` defaulting to 24. The check is that the checksum still has the width it claims |
| **The row limit, against a newline** | Rendered lines checked for control characters, not only for width | **Found a defeat of the display invariant.** `check_fits` counts list entries as rows and characters as width, and a newline is one character, so a string containing one passes both checks and the panel draws two rows in the space of one. Gate rejection messages contain a newline by construction ("Rejected at T3.\n72 bpm is outside…"), so this was every failed capture. `wrap_full` splits on newlines now and both `check_fits` functions refuse control characters |
...
| **T5, against a metronome across rates** | The synthetic panel's genuine sessions were all identical; made to vary in rate, perfusion depth, ratio and DC level | **Found the shipped `rmssd_min_ms` too low.** A perfectly regular pulsator reads 0.8–7.0 ms depending on how its period lands on the 50 Hz grid, peaking near 69 bpm; genuine sessions read 43–88 ms. The floor was 5.0, so a metronome at 69 bpm passed T5, and the README's own claim is that "a mechanical pulsator produces single digits", which 5 admits. Raised to 15. Synthetic, so a corrected starting point and not a bench measurement; what is not synthetic is that 5 contradicted the separation the gate is described as making |
| **The bench tool's own arithmetic** | `bench.py selftest`. The bounce measurement and the PIN-budget comparison, without the parts | **Found two false alarms**, both in checks a builder runs once on a bench and would have believed. The bounce measurement spanned press *through release*, reporting how long a finger was down (100–300 ms) against a 30 ms debounce, failing every switch ever made, and advising a debounce past a fifth of a second on the button that means consent. The second: "a correct PIN restores the budget" compared against the reading taken when the tool started, which is lower on any chip that has ever seen a wrong PIN |
...
| **Stopping a capture early** | `test_gate_robustness.py` runs all 18 spoof classes twice, once with the aborts armed and once out to the full duration, and compares the two verdicts gate by gate | Identical on every class. `acquire()` now leaves at G5 as well as at chemistry, because G5 reads only the frames inside `early_window_s` and cannot change afterwards. Five classes take that exit, the three aged samples, `rewarmed`, and the sealed REFERENCE cartridge, which is what takes the §5 pre-flight off a ten-minute wait. **Found one defect in the abort itself:** a stalled camera returns bit-identical frames, `speckle_metrics` reports NaN, and G5 reads that as "did not move" — so the first version left at 60 s carrying G5's sentence and sent the owner to look at their blood instead of at the CSI cable. The abort now stands aside for a non-finite series and lets G6 name the sensor |
| **The cartridge switch, against its own two jobs** | Read off BUILD.md §9 and §11 together: the switch has to pull GPIO22 down when a cartridge is seated, and carry the laser's supply so an open bay cannot be lit | **Found a build-blocking conflict.** Both jobs want a contact that closes on seating, and BOM.csv bought an SPDT, which has one pole and therefore one such contact. A builder wiring §11 gets no interlock; one wiring §9 gets no cartridge sense; tying them together puts the diode's rail on a 3V3 input the moment the bay opens. Now a DPDT, one pole each, $0.50 more. Not machine-checked, unlike the rows above it, because nothing here models a contact |
| **The spec tables, against the code** | Every threshold in BUILD.md's G1–G6 and T0–T6 tables, and §3's restatement of the motion gates, parsed and compared against `Thresholds` and `TouchThresholds`. Both itemised kit tables and §6's parts table summed and compared against `BOM.csv`. The printable-part count against `models/print/`, the screw length against `gen_enclosure.SCREW_LEN`, and the switch body `BOM.csv` buys against the button pitch the deck is drilled to | **Found six stale numbers**, all of them in the documents somebody builds and orders from. T5's floor still read 5 ms after the row above raised it to 15, so BUILD.md was publishing the threshold a metronome walks through. §6 sold M2.5×8 screws, which stop a millimetre below the part line and never reach the insert. The wallet kit's rows added to $35.30 under a $34.70 heading, and §6's table was missing the diffuser, the epoxy and the breadboard. README counted ten printable parts against eleven on disk. And the BOM bought four 12 mm switches for a deck drilled on an 11 mm pitch, where three of them foul each other and none of their caps passes the hole. Each check verified to bite by planting the number it is meant to catch |
...
The ATECC608B check tests behaviour instead of reading the config zone back.
